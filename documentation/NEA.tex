...
class Engine:
    ...
    def get_asset(self, path, audio=False, font=False, scale=False):
        ...
        # If it's not in the file system map return a default to prevent errors
        if path in self.__fs_map:
            path = self.__fs_map[path]
        else:
            self.append_log(f"Error! Asset {path} was not found in file map! Changing files requires running self.map_fs() again!")
            if audio:
                return self.__assets["missing.ogg"]
            elif font:
                return self.__assets["ComicMono.ttf"]
            else:
                return self.__assets["missing.png"]

        # If it's cached we don't need to do anything else
        if path in self.__assets:
            if audio or font:
                return self.__assets[path]
            elif scale and scale in self.__assets[path]:
                # If we have the scaled version return it
                return self.__assets[path][scale]
            elif not scale:
                return self.__assets[path]["raw"]
            else:
                # Otherwise scale it
                self.__assets[path][scale] = self.__scale(self.__assets[path]["raw"], scale, path)
                return self.__assets[path][scale]

        # Fetch if not in cache
        if audio:
            try:
                self.__assets[path] = pygame.mixer.Sound(path)
                return self.__assets[path]
            except:
                self.append_log(f"Error! Asset {path} was not found in the mapped location! Changing files requires running self.map_fs() again!")
                return self.__assets["missing.ogg"]
        elif font:
            try:
                self.__assets[path] = pygame.font.Font(path)
                return self.__assets[path]
            except:
                self.append_log(f"Error! Asset {path} was not found in the mapped location! Changing files requires running self.map_fs() again!")
                return self.__assets["ComicMono.ttf"]
            ...
            try:
                self.__assets[path] = {"raw": pygame.image.load(path)}
                if not scale:
                    return self.__assets[path]["raw"]
                else:
                    self.__assets[scale] = self.__scale(self.__assets[path]["raw"], scale, path)
                    return self.__assets[scale]
            except:
                self.append_log(f"Error! Asset {path} was not found in the mapped location! Changing files requires running self.map_fs() again!")
                return self.__assets["missing.png"]["raw"]


    def __scale(self, obj, scale, path):
